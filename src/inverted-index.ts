...
export function createSearchableText(data: any): string {
  ...
  if (data.bestPracticeStandards) {
    ...
    if (standards.qualityStandards) {
      ...
      if (quality.accessibilityImplementation) parts.push(quality.accessibilityImplementation.join(' '));
      if (quality.designPatternUsage) parts.push(quality.designPatternUsage.join(' '));
      if (quality.userExperienceGuidelines) parts.push(quality.userExperienceGuidelines.join(' '));
    }
  }
