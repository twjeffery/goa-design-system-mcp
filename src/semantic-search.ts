...
export class SemanticSearchEngine {
  ...
  public detectIntent(query: string): QueryIntent {
    ...
    return {
      type: primaryIntent.type as any,
  ...
  private getSuggestedTools(intentType: string): string[] {
  ...
  private getContextBoosts(intentType: string): Record<string, number> {
   ...
   */
  public expandQuery(originalQuery: string): string[] {
   ...
   */
  public calculateSemanticSimilarity(query: string, content: any): number {
   ...
   */
  public calculateFuzzySimilarity(str1: string, str2: string): number {
      ...
      for (const field of searchableFields) {
        if (content[field]) {
          if (Array.isArray(content[field])) {
            textParts.push(...content[field].map((item: any) => String(item)));
          } else {
  ...
  public enhancedSearch(
    query: string, 
    dataItems: Array<{name: string, content: any, type: string}>,
    intent: QueryIntent
  ...
  private calculateRelevanceFactors(
    ...
    item: {name: string, content: any, type: string},
    intent: QueryIntent
  ) {
  ...
  private calculateIntentAlignment(
    ...
    const typeAlignmentMap: Record<string, Record<string, number>> = {
      'component_search': {
        'component': 1.0,
        'system': 0.3,
        'workflow': 0.2
